...
static FILE* file_error ("log\\input_output.txt", "w");

int get_database (struct token** root, char* file_input)   // get data of tree in the following file
{
    CALLOC (*root, struct token, 1);
...
int construct_data_nodes (struct token* root, char* text_data, size_t file_size)
{
    ...
    for (int i = 0, n = 0, position = ROOT; i < file_size; i++)
    {
        if (text_data[i] == '(' && position == ROOT)  // add a root of the tree
        {
           ...
           //      i++;
            ...
            while (text_data[i] != '(' && text_data[i] != ')')               // fill buffer with a new word
            {
